...
@router.post("/delete")
async def delete_game_post(payload: IdSchema, current_user: UserOut = Depends(get_current_user)):
    await delete_game_service(payload.id, user_id=current_user.id)
    return {"id": payload.id, "message": "Game deleted successfully!"}


@router.post("/update")
async def update_game_post(game_data: GameUpdate, current_user: UserOut = Depends(get_current_user)):
    await update_game_service(game_data, user_id=current_user.id)
...
async def add_wishlist_post(payload: IdSchema, current_user: UserOut = Depends(get_current_user)):
    await add_wishlist_service(payload.id, user_id=current_user.id)
    return {"id": payload.id, "message": "Game added to wishlist!"}

@router.post("/buy")
async def buy_game_post(payload: IdSchema, current_user: UserOut = Depends(get_current_user)):
    await buy_game_service(payload.id, user_id=current_user.id)
    return {"id": payload.id, "message": "Game purchased successfully!"}

@router.post("/cart")
async def add_cart_post(payload: IdSchema, current_user: UserOut = Depends(get_current_user)):
    cart = await add_cart_service(payload.id, user_id=current_user.id)
    return cart
